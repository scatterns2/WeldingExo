...
Script.Load("lua/Exo_Modular.lua")
...
//local kFireLoopingSound = PrecacheAsset("sound/NS2.fev/marine/grenades/pulse/explode")
...
function Exo:InitWeapons()
	...
	self.scannerEnt = CreateEntity(ExoScan.kMapName, self:GetOrigin(), self:GetTeamNumber())        
	self.scannerEnt:SetParent(self)
...
if Client then
    ...
    function Exo:OnUpdateRender()
        ...
        if self.isShieldActive then
            ...
            //self:TriggerEffects("shield_on", params)    
        ...
        //   self:TriggerEffects("shield_deploy", params)    
...
function Exo:HandleButtons(input)
	...
	if bit.band(input.commands, Move.Weapon2) then
		...
		self.wasScannerButtonDown = true
		
	else
	
		self.wasScannerButtonDown = false
...
GetEffectManager():AddEffectData("ShieldEffects", {
   shield_deploy =
    {
        shieldEffects =
        {   
   ...
   shield_on =
    {
        shieldOnEffects =
        {
                {cinematic = "cinematics/marine/welder/shield_light.cinematic", done = true},

        },
      },  
})

Shared.LinkClassToMap("Exo", Exo.kMapName, networkVars, true)// ======= Copyright (c) 2012, Unknown Worlds Entertainment, Inc. All rights reserved. ============
//
// lua\Exo.lua
//
//    Created by:   Brian Cronin (brianc@unknownworlds.com)
